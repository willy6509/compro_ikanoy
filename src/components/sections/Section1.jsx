...
export default function Section1() {
  ...
  useGSAP(() => {
    ...
    // Teks akan perlahan hilang saat discroll ke bawah, dan muncul lagi saat ke atas
    gsap.to(".hero-content", {
      scrollTrigger: {
        trigger: "#home",
        start: "top top",      // Mulai saat bagian atas section di atas layar
        end: "bottom center",  // Selesai saat bagian bawah section di tengah layar
        scrub: true,           // Animasi terikat dengan gerakan jari/mouse (halus)
      },
      y: -150,    // Geser ke atas
      opacity: 0, // Memudar jadi transparan
      ease: "none"
    });

  }, { scope: heroRef });

  // Fungsi Scroll Halus ke Produk (Mencegah URL berubah jadi /#product)
    ...
    <section ref={heroRef} id="home" className="relative flex min-h-screen w-full items-center justify-center pt-20 overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-end justify-center pointer-events-none -z-10 w-full h-full">
        <img
          src="/images/wave.png"
          alt="wave background"
          className="wave-layer absolute bottom-0 left-[-5%] w-[110%] max-w-none h-auto object-cover opacity-80" 
        />
      </div>

      {/* === KONTEN UTAMA === */}
      {/* Class 'hero-content' ditambahkan untuk target animasi scroll */}
